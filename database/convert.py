...
for _, row in df.iterrows():
	...
	for subject in subjects:
		score_val = row[en2vi[subject]] if pd.notna(row[en2vi[subject]]) else None
		lang_code = row['ma_ngoai_ngu'] if subject == 'foreign_language' and pd.notna(row['ma_ngoai_ngu']) else None
